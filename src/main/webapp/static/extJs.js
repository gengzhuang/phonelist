...
$.extend($.fn.validatebox.defaults.rules, {
	...
	mobile : {
		validator : function(value, param) {
			return /^(?:13\d|15\d|18\d|17\d|14\d)-?\d{5}(\d{3}|\*{3})$/.test(value);
		},
		message : '手机号码不正确。'
	},
	tel : {
		validator : function(value, param) {
			return /^(\d{3}-|\d{4}-)?(\d{7})?(-\d(1,6))?$/.test(value);
		},
		message : '请输入正确的格式，如：0317-1234567'
	},
	year : {
		validator : function(value, param) {
			return /(19[\d][\d]|20[\d][\d])/.test(value) && value.length <= 4;
		},
		message : '请输入正确四位年份，1900-2099'
	},
	month : {
		validator : function(value, param) {
			return value >=1 && value <= 12;
		},
		message : '请输入正确月份，1-12'
	},
	equalTo: {
        validator:function(value,param){
